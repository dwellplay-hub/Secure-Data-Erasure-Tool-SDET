...
class SDETApp(ctk.CTk):
    def __init__(self):
        ...
        self._selected_path = tk.StringVar(value="") # Visual path untuk UI
        self._real_target_path = ""                  # Backend path sebenar (RAHSIA)
    ...
    def _browse_file(self):
        ...
        if path:
            self._real_target_path = path  # Simpan laluan rahsia di memori belakang
            masked_name = _mask_filename(os.path.basename(path))
            self._selected_path.set(f"...\\{masked_name}") # Papar topeng di UI
            
            self._is_directory = False
            self._recursive.set(False)
            self._log(f"Selected file: {masked_name}", COLOR_INFO)
    ...
    def _browse_dir(self):
        ...
        if path:
            ...
            self._log(f"Selected folder: {masked_name}", COLOR_WARNING)
    ...
    def _start_erase(self):
        # Gunakan self._real_target_path (bukan yang ditopengkan dari UI)
        path = self._real_target_path.strip()
        
        if not path or not self._selected_path.get().strip():
            messagebox.showerror("No Target", "Please select a file or folder first.")
            return

        if not os.path.exists(path):
            messagebox.showerror("Not Found", "Target does not exist on disk.")
            return
        ...
        # MASK NAMA FAIL DI DALAM POPUP AMARAN
        masked_target = _mask_filename(os.path.basename(path))
        ...
        self._log(f"Starting erasure: {masked_target} [{method_name}]", COLOR_WARNING)
    ...
    def _run_directory_erase(self, path: str, method: str, progress):
        ...
        if results and results[0].get("status") == "BLOCKED_BLACKLIST":
            self.after(0, lambda: messagebox.showwarning(
                "Security Blocked",
                "SECURITY WARNING: This directory is on the Blacklist because it contains critical system files.\n\nThe operation has been automatically aborted to protect Windows."
            ))
    ...
    def _on_erase_complete(self):
        ...
        self._real_target_path = "" # Reset memori laluan rahsia
